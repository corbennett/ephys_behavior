...
for expname in data.keys():
    print('tensorizing ' + expname)
    d = data[expname]
    probes = d['units'].keys()
    
    if 'passiveFlashTimes' not in d:
        continue
    ...
    # SELECT GOOD TRIALS (OMIT CHANGE and REWARD CONSUMPTION?)
...
#    saveDir = r"\\allen\programs\braintv\workgroups\nc-ophys\corbettb\ccg tensors"
    saveDir = r"C:\Users\svc_ccg\Desktop\Data\tensors"
    np.savez_compressed(os.path.join(saveDir, expname+ '.npz'), imageID=imageID, active_tensor=tensor_active, passive_tensor=tensor_passive, ccfRegions=np.array(unitRegion))
